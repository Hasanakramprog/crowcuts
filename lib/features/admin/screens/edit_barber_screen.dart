...
class _EditBarberScreenState extends ConsumerState<EditBarberScreen> {
  ...
  void initState() {
    super.initState();
    final barber =
        widget.barberId != null ? ref.read(barberProvider(widget.barberId!)) : null;

    _nameController = TextEditingController(text: barber?.name ?? '');
    _phoneController = TextEditingController(text: '');
    _emailController = TextEditingController(text: '');
    _experienceController =
        TextEditingController(text: barber?.experienceYears.toString() ?? '');
    _isActive = barber?.isActive ?? true;
    // Populate selected services + pricing controllers
    _selectedServiceIds = barber?.services.map((s) => s.serviceId).toList() ?? [];
    for (final svc in barber?.services ?? []) {
      _priceControllers[svc.serviceId] =
          TextEditingController(text: svc.price.toStringAsFixed(0));
      _durationControllers[svc.serviceId] =
          TextEditingController(text: svc.durationMinutes.toString());
    }
  ...
  Future<void> _save() async {
    ...
    final barberServices = _selectedServiceIds.map((id) {
      ...
      return BarberService(
        ...
        durationMinutes: int.tryParse(_durationControllers[id]?.text ?? '') ?? 30,
      );
    }).toList();
    ...
    if (widget.barberId != null) {
      ...
      if (existing != null) {
        ref.read(barbersProvider.notifier).updateBarber(existing.copyWith(
        ...
        if (_linkedUserId != null) {
          ...
          if (existingUser != null) {
            await repo.updateUser(existingUser.copyWith(
      ...
      // Adding — create new barber
      final uuid = const Uuid();
      final newBarber = BarberModel(
        id: 'barber-${uuid.v4().substring(0, 8)}',
        name: _nameController.text.trim(),
        rating: 0.0,
        reviewCount: 0,
        experienceYears: int.tryParse(_experienceController.text) ?? 0,
      ...
      ref.read(barbersProvider.notifier).addBarber(newBarber);
    }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(widget.barberId != null
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Select which services this barber provides',
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text('No services available. Add services first.',
                                ...
                                children: [
                                  Expanded(
                                    child: TextField(
                                      ...
                                      keyboardType: TextInputType.number,
                                      style: AppTypography.body.copyWith(
                                        color: context.isDark ? AppColors.goldPrimary : context.colors.textPrimary,
                                      ),
                                    ),
                                  ),
                                  const SizedBox(width: 12),
